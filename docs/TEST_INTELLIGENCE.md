...
|---|------|----------|----------|-------------|----------|
| 1 | §7 任务化、§3.5 执行、§8 M3 | analyze/run 复用任务状态机，`tasks` 新增 `kind=test-run/env/audit/fix-apply` | `store.Task` **无 `Kind` 字段**、`tasks` 表无 kind 列；5 处生产建任务调用点无一处来自 intel；intel 走 `intel_runner.go` 自有派发 + `intelSem` + `intelExecMutex` + `intelCancels`。**短期项已做（2026-09-20）**：`FailStaleIntelRuns` 启动期把遗留 `running/queued` 置 failed | intel run **仍拿不到重试、`dependsOn`、优先级、worker 池、`RecoverStaleRunning` 崩溃恢复、janitor 清理**（孤儿悬挂已通过启动恢复缓解） | 完整任务化（test-run kind）保留为长期待办；孤儿悬挂已修复 |
