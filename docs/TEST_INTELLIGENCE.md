...
| 14 | 文首设计原则、§3.1 Profile 注册表 | "现装现有体系：Java/Maven、Go、Android、iOS、Web、BFF(GraphQL)、Node" | **已实现**：类型探测（`profile.go` 7 个 Profile 锚点）全有；结构化扫描器已覆盖 **Java（`java.go`）+ Go（`go.go`）+ Android（`android.ScanKotlin`）+ iOS（`ios.ScanSwift`）+ Web（`web.ScanWeb`）**，`ScanModule` 按锚点分派（xcodeproj/Package.swift→ios、package.json→web、.kt→android、go.mod→go、否则 java） | 混合仓库 Android/iOS/Web 子项目已能拿到实体/端点契约；BFF(GraphQL)/Node 结构化扫描仍未专设（web 走 package.json 前端契约） | ✅ Java/Go/Android/iOS/Web 扫描器已接线；BFF/Node 契约扫描留待 |
