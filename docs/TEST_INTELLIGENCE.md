...
GET  /api/intel/modules?projectId={id}        仓库内子项目清单（monorepo 识别结果，含 type/role）
GET|PUT /api/intel/modules/{id}               模块详情/审核命令白名单（按 moduleId 操作，GET 取详情、PUT 改 commands/role）
...
POST /api/intel/issues/{id}/ack              人工标记/复核（resolved，附 resolved_at）
POST /api/intel/issues/{id}/link-feature    将问题关联到功能点（挂 issue 于 feature）
...
|---|------|----------|----------|-------------|----------|
| 1 | §7 任务化、§3.5 执行、§8 M3 | analyze/run 复用任务状态机，`tasks` 新增 `kind=test-run/env/audit/fix-apply` | **已实现（2026-09-21）**：`store.Task` 有 `Kind` 列（迁移 `task_kind`）；run-all 下沉为 `kind=test-run` 追踪任务**交给共享 executor 的 test-run 专用 worker** claim 执行（`ClaimNextTaskOfKind`、`WithIntelRunner` 注入回调 `RunIntelAllTask`），复用 executor 的 worker 池/priority/dependsOn/`RecoverStaleRunning`/重试/janitor；单模块 run（`runIntelJob`）仍走自有轻量派发；入队层 envGate 加「有匹配节点豁免」（`hasRemoteNodeFor`/`hasAnyRemoteNode`） | 单模块 run 仍未入共享 executor（保留自有轻量替代）；scope=all 已完整任务化 | ✅ 完整任务化已兑现（run-all 接入共享 executor）；单模块 run 仍走轻量路径 |
| 2 | §3.6、§7 执行 transport、§10 iOS | 远程执行用 `golang.org/x/crypto/ssh`（"已在依赖中"）、流式回传、scp 拉回产物、按能力标签路由 | **已实现（2026-09-21）**：仍无 `crypto/ssh` import、`envagent.RunSSH` 起系统 `ssh` 二进制 + `CombinedOutput()`；但**新增 `envagent.PullArtifacts`/`PullJUnitXML`（`pull.go`）** 用 `tar -czf - … 2>/dev/null` + 内存解包（`archive/tar`+`compress/gzip`）把远端报告拉回；**放开 `reportKind != "go"` 拒绝**——surefire/pytest/xctest 落盘报告回拉到临时目录交给 `parseReport` 本地解析；**`capabilities` 路由**已接线：`remoteCapabilityFor` 按 buildTool 推导所需标签（go→go/maven→java/gradle/android/node/python/ios），`nodeHasCapability` 前缀匹配，缺则拒绝并给中文错误，空 capabilities 视为未标注、放行 | 去 macOS/远端跑 Maven/Gradle/pytest 的整条路已通；剩余：`@end/` 多仓库模块的远程相对路径未特殊处理、capabilities 仍为标签匹配而非自动选节点、依赖节点装有 `tar` | 长期可按需引入 `crypto/ssh` 流式回传替代 tar-over-ssh；`@end/` 远程路径对齐多仓库根留待后续 |
| 3 | §6 全局设置、§3.1 缓存路径、§5 | `intel.repos_dir` 两阶段修改（预检 → 确认 → 破坏性删除重建），默认路径 `~/.local/share/starburst-backend/intel-repos` | **已实现（2026-09-20，db942c9）**：`intel_repos.go` 有 `handleIntelReposProbe`/`handleIntelReposRebuild` 两阶段端点；`intelDefaultReposDir()` 提供默认路径，读不到 setting 时回退默认值 | 改路径会清空旧缓存（repos-rebuild 破坏性删除），未配置时用默认路径可分析 Git 项目 | ✅ 两阶段端点 + 默认值均已兑现 |
| 4 | §6/§7 测试并发、§4 `intel.workers` | 并发数页面可配、默认 1（串行）、热生效不重启 | **已实现（2026-09-20/21）**：`intelSem` 动态信号量（`intel_runner.go`）替代固定容量 channel；`intel.workers` setting 驱动，`GET/POST /api/intel/settings` 可调、启动时恢复、cap 变化即时生效；**前端设置项已对接**（`index.html` 并发输入 + `saveIntelWorkersSetting`/`loadIntelWorkersSetting`，页面加载即填充） | 默认仍 `intelExecConcurrency=2` 并行（与文档"默认 1"不同，已改口径） | ✅ 并发可配已兑现（后端 + API + 前端设置项） |
| 5 | §7 推送 | 9 个新事件类型（`intel.ready`…`feature.chat.answer`） | **已实现（2026-09-21）**：新增 `internal/server/intel_events.go` 的 `pushIntelEvent` + 事件常量，在 `envGate`/`intel_audit.go`/`intel_fix`/`intel_feature_chat.go` 发出语义事件：`intel.gate.blocked`、`intel.env.ready`、`intel.audit.finding`、`intel.fix.suggested`、`intel.fix.applied`、`intel.feature.chat.answer`（保留 `intel.run.event` 兼容）；前端 `app.js` 对应消费 | 前端可按语义区分环境就绪/门禁拦截/审计发现/修复建议/对话回复并即时刷新 | ✅ 语义事件已补（6 类 + 保留 run.event）；如需更多细分语义可再扩 |
| 6 | §7 自动化、§7 批量 | `rules.kind` 扩展 `intel-run`；`/api/batch` 可并行下发分析/执行 | **已实现（2026-09-20/21）**：rules 加 `intel_project_id` 列（迁移 `intel_rule_project`）、`Rule.IntelProjectID` 字段；`Engine.Fire` 命中时通过 `SetIntelRunner` 回调触发 `TriggerIntelRunAll`；`/api/rules` POST 支持 `intelProjectId`；**`/api/batch` 已支持 `intelProjectIds`**（batch.go，与 prompt/targets 互斥，批量触发各项目 run-all 回归返回 runIds，上限 100） | cron/git/http 规则可自动触发测试智能回归（§8 M7 验收成立）；批量接口可一次下发多个项目回归 | ✅ 规则→intel-run 自动化 + /api/batch intel 批量均已实现 |
| 7 | §3.5/§6 flaky | 连续 N 次先败后绿 → `flaky_quarantined`、自动剔除出增量范围、页面隔离区可解除 | **已实现（2026-09-20/21）**：test_cases 加 `quarantined` 列（迁移 `intel_flaky_quarantine`），`UpdateIntelTestCaseOutcome` 在 flaky_count 累计达阈值（3 次）时置隔离；`flakyRetry`/`recordRunIssues` 跳过已隔离用例；`POST /api/intel/test-cases` 解除隔离。**跨报告类型 flaky（2026-09-21）**：`flakyRetry` 泛化到 **go/surefire(maven+gradle)/pytest/playwright**；playwright 用 `--grep ^QuoteMeta(fullTitle)$` 锚定（FullTitle=project+file+describe 链+test，经 FailuresJSON 的 `fullTitle` key 穿过 store，消除 describe 链丢失）；**xctest 在命令带 `-scheme` 时支持**（`-only-testing:Class/method`，classname 最后一个点拆方法名）；npm 仍 return early（无逐用例选择器） | 不稳定用例达 3 次后被隔离并告警、可解除，跨 go/surefire/pytest/playwright 生效，xctest 依赖命令携带 -scheme；xcodebuild 默认不产 JUnit，重跑解析依赖项目自身落盘 junit 配置（`collectJUnitXML` 兜底，属外部工具行为） | ✅ 隔离状态机 + 跨报告 flaky 判定已兑现（2026-09-21 加深：full-title 锚定 + spec.file 对齐 + xctest scheme 透传） |
| 8 | §5、§3.7 SBOM 与扫描触发 | `POST /api/intel/sbom`、`POST /api/intel/scan/findings`、`/api/intel/sync/scan`、`/api/intel/issues/{id}/ack`、`/api/intel/issues/{id}/link-feature`、`/api/intel/runs/{id}/results` | **已实现（2026-09-21）**：`intel_scan_trigger.go`（`POST /api/intel/sbom` 复用 `persistOverview` 重生成、`POST /api/intel/scan/findings` 与 `/sync/scan` 触发合规/安全扫描）+ `intel_issues.go`（`/{id}/ack` 标记 resolved、`/{id}/link-feature` 关联功能点）+ `handleIntelRunByID` 的 `GET /runs/{id}/results`；store 新增 `MarkIntelIssueResolved`/`LinkIntelIssueFeature`（复用现有列，无迁移） | 依赖本节接口表的调用方可正常使用；SBOM 下载仍可走前端 `downloadIntelSbom()`（读 overview.sbom_json） | ✅ 全部端点已注册（2026-09-21） |
...
| 10 | §3.1/§6/§9 Git 凭据 | clone 认证 HTTP token / SSH 私钥加密落库、运行时注入 | **已实现（2026-09-20，889b659）**：git token/ssh key AES-GCM 加密落库；`intel_workspace.go` 经临时 `GIT_ASKPASS`/`GIT_TERMINAL_PROMPT=0` 注入 clone/fetch（中间件口令与节点 SSH auth 同已加密落库） | 私有 GitLab 仓库可页面配置凭据并克隆 | ✅ Git 凭据加密落库 + 注入已兑现 |
| 11 | §3.6 依赖声明 | 人工 manifest `intel-env.yaml`（含 `init_scripts`） | **已实现（2026-09-20，6c35e6b）**：`envdetect/manifest.go` 的 `ParseManifest`/`ManifestAt` 解析 `intel-env.yaml`（services→requirements、init_scripts），`intel_persist.go`/`env.go` 合并人工声明覆盖自动推导 | 自动推导覆盖不到的项目可人工补声明 | ✅ manifest 解析 + 合并 + init_scripts 接线已兑现 |
| 12 | §5 路径归属 | `GET /api/intel/projects/{id}/modules`、`PUT …/commands`、`…/commands-modules` | 实际是 `GET /api/intel/modules?projectId=`（子项目清单）与 `GET\|PUT /api/intel/modules/{id}`（按 moduleId 取详情/改命令），projects 子路径只挂了 `sources`；**§5 路径表已改写（2026-09-21）** | 与 §5 原写法不一致，易误判为缺失 | ✅ §5 路径表已对齐代码实际路由 |
| 13 | §3.5、§8 M3 报告解析 | 解析 surefire / playwright json / go test -json（§3.5 另列 XCTest、pytest） | **已实现（2026-09-20）**：`parseReport` 加 `playwright`/`xctest`/`pytest` 分派；`detectPlaywright` 检出 web/node 项目默认 `npx playwright test --reporter=json`；report 包新增 `ParseXCTestJUnit`/`ParsePytestJUnit`（子代理完成，有单测）；python Profile + BuildTool=pytest 注册（`pytest --junitxml`）；xcode BuildTool 的 reportKind=xctest、命令靠白名单/计划（无 -scheme 无法默认） | Web/E2E + Python + iOS(XCTest) 逐用例结果可解析；surefire/go 不变 | ✅ Playwright/XCTest/pytest 均已接线；pytest 报告位置固定 junit.xml，xcode 需人工配命令 |
| 14 | 文首设计原则、§3.1 Profile 注册表 | "现装现有体系：Java/Maven、Go、Android、iOS、Web、BFF(GraphQL)、Node" | **已实现**：类型探测（`profile.go` 7 个 Profile 锚点）全有；结构化扫描器已覆盖 **Java（`java.go`）+ Go（`go.go`）+ Android（`android.ScanKotlin`）+ iOS（`ios.ScanSwift`）+ Web（`web.ScanWeb`）+ BFF/Node（`bff.ScanBFF`：GraphQL SDL + REST 路由提取，2026-09-21）**，`ScanModule` 按锚点分派（xcodeproj/Package.swift→ios、package.json→bff/web 判断、.kt→android、go.mod→go、否则 java） | 混合仓库 Android/iOS/Web 子项目已能拿到实体/端点契约；BFF/Node 走 bff 包（GraphQL 实体+端点、REST 路由），并与 web 前端绑定合并保留；限界见 bff 包注释（gql 模板嵌套/动态路由/union-scalar 不解析，属零依赖正则扫描的设计取舍） | ✅ Java/Go/Android/iOS/Web/BFF(Node) 扫描器已接线；BFF/Node 契约扫描轮已补（2026-09-21） |
