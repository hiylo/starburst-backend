...
|---|------|----------|----------|-------------|----------|
| 1 | §7 任务化、§3.5 执行、§8 M3 | analyze/run 复用任务状态机，`tasks` 新增 `kind=test-run/env/audit/fix-apply` | `store.Task` 已有 `Kind` 列（迁移 `task_kind`），intel 仍走 `intel_runner.go` 自有派发 + `intelSem` + `intelExecMutex` + `intelCancels`，不进共享 executor（`ClaimNextTask` 过滤 `kind=''`）。**已补齐（2026-09-20/21）**：`FailStaleIntelRuns` 启动恢复、`runIntelJob` 执行失败自动重试（`intelMaxRunAttempts`）、run-all 下沉为 `kind=test-run` 追踪任务、`test_runs.priority` 列（迁移 `test_runs_priority`）+ run-all 按优先级排模块、`PurgeOldIntelTestRuns` janitor（30 天保留/每批 500，级联删 test_results） | intel run 仍未复用共享 executor 的 worker 池与 `RecoverStaleRunning`/`dependsOn` 全自动链路（用自有轻量替代） | 完整任务化（test-run kind 入共享 executor）保留为长期待办；优先级/janitor/崩溃恢复/重试已以轻量版兑现 |
...
| 7 | §3.5/§6 flaky | 连续 N 次先败后绿 → `flaky_quarantined`、自动剔除出增量范围、页面隔离区可解除 | **已实现（2026-09-20/21）**：test_cases 加 `quarantined` 列（迁移 `intel_flaky_quarantine`），`UpdateIntelTestCaseOutcome` 在 flaky_count 累计达阈值（3 次）时置隔离；`flakyRetry`/`recordRunIssues` 跳过已隔离用例；`POST /api/intel/test-cases` 解除隔离。**跨报告类型 flaky（2026-09-21）**：`flakyRetry` 泛化到 go/surefire(maven+gradle)/pytest，新增纯函数 `flakyRerunArgs`（maven `-Dtest=Class#method`、gradle `--tests`、pytest `-k`）；前端隔离区视图已存在（`app.js` 已隔离标记 + 解除按钮） | 不稳定用例达 3 次后被隔离并告警、可解除，跨 go/surefire/pytest 生效；playwright/xctest/npm 仍 return early（重跑选择器不具确定性） | ✅ 隔离状态机 + 跨报告 flaky 判定已兑现；playwright/xctest 重跑留待（缺稳定选择器） |
