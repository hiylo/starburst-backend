...
| 6 | §7 自动化、§7 批量 | `rules.kind` 扩展 `intel-run`；`/api/batch` 可并行下发分析/执行 | **已实现（2026-09-20）**：rules 加 `intel_project_id` 列（迁移 `intel_rule_project`）、`Rule.IntelProjectID` 字段；`Engine.Fire` 命中时通过 `SetIntelRunner` 回调触发 `TriggerIntelRunAll`（intel run-all 回归）而非 prompt 任务；`/api/rules` POST 支持 `intelProjectId` | cron/git/http 规则可自动触发测试智能回归（§8 M7 验收成立）；`/api/batch` 仍只建 prompt 任务 | ✅ 规则→intel-run 自动化已实现；batch 扩展待补 |
