...
func (s *sqlStore) DeleteIntelProject(ctx context.Context, id int64) error {
	stmts := []string{
		...
		`DELETE FROM intel_feature_chats WHERE project_id = ?`,
		`DELETE FROM intel_overrides WHERE project_id = ?`,
		...
		`DELETE FROM env_requirements WHERE project_id = ?`,
		`DELETE FROM env_services WHERE project_id = ?`,
		`DELETE FROM env_devices WHERE project_id = ?`,
