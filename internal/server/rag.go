...
func (s *Server) buildIntelChunks(ctx context.Context, projectID int64) ([]*store.RagChunk, error) {
	...
	// Web client bindings become queryable facts as well.
	if bindings, err := s.store.ListIntelWebBindings(ctx, projectID); err == nil {
		byPage := map[string][]*store.IntelWebBinding{}
		var pageOrder []string
		for _, b := range bindings {
			if _, ok := byPage[b.Page]; !ok {
				pageOrder = append(pageOrder, b.Page)
