...
func Parse(args []string) (*Config, error) {
	...
	listenAddr := fs.String("listen", envOr("STARBURST_LISTEN", ":18880"), "HTTP listen address")
	opencodeURL := fs.String("opencode-url", envOr("STARBURST_OPENCODE_URL", "http://127.0.0.1:4096"), "local OpenCode server base URL")
	dbDriver := fs.String("db", envOr("STARBURST_DB", "sqlite"), "database driver: sqlite or postgres")
	sqlitePath := fs.String("sqlite-path", envOr("STARBURST_SQLITE_PATH", "starburst-backend.db"), "SQLite database file path")
	postgresDSN := fs.String("pg-dsn", os.Getenv("STARBURST_PG_DSN"), "PostgreSQL connection string")
	defaultAdmin := fs.String("default-admin-password", envOr("STARBURST_ADMIN_PASSWORD", "admin"), "default web admin password (used only on first initialization)")
	defaultToken := fs.String("default-token", os.Getenv("STARBURST_DEFAULT_TOKEN"), "optional pre-provisioned API token registered on first run (empty = off)")
	webhookSecret := fs.String("webhook-secret", os.Getenv("STARBURST_WEBHOOK_SECRET"), "optional shared secret protecting /api/webhook (empty = off)")
	workers := fs.Int("workers", envInt("STARBURST_WORKERS", 4), "concurrent task executions (1 = serial)")
	maxConcurrency := fs.Int("max-concurrency", envInt("STARBURST_MAX_CONCURRENCY", 0), "global running task cap across workers (0 = unlimited within workers)")
	taskRetention := fs.Duration("task-retention", envDuration("STARBURST_TASK_RETENTION", 0), "finished task retention, 0 = forever")
	llmURL := fs.String("llm-url", envOr("STARBURST_LLM_URL", ""), "OpenAI-compatible base URL for orchestration LLM (empty = disabled)")
	llmKey := fs.String("llm-key", os.Getenv("STARBURST_LLM_KEY"), "API key for --llm-url")
	llmModel := fs.String("llm-model", envOr("STARBURST_LLM_MODEL", ""), "model name for orchestration decisions")
	embedURL := fs.String("embed-url", envOr("STARBURST_EMBED_URL", ""), "OpenAI-compatible base URL for embeddings (empty = disabled)")
	embedKey := fs.String("embed-key", os.Getenv("STARBURST_EMBED_KEY"), "API key for --embed-url")
	embedModel := fs.String("embed-model", envOr("STARBURST_EMBED_MODEL", ""), "embedding model name (e.g. bge-m3)")
	...
	if driver == "postgres" && strings.TrimSpace(*postgresDSN) == "" {
		return nil, fmt.Errorf("db=postgres requires a postgres DSN (--pg-dsn or STARBURST_PG_DSN)")
	}
